...
import { setOwner, onlyOwner, ownerAddress } from '../utils/ownership';
import {
  Storage,
  generateEvent,
  transferCoins,
...
export const MINT_START_TS = stringToBytes('MINT_START_TS');
...
export function constructor(binaryArgs: StaticArray<u8>): void {
  ...
  Storage.set(MINT_START_TS, u64ToBytes(1720807200000)); // https://www.epochconverter.com/ Friday 12 July 2024 18:00:00

  generateEvent('MASSAMON COLLECTION IS DEPLOYED');
}
...
export function setMintStart(_args: StaticArray<u8>): void {
  onlyOwner();
  const args = new Args(_args);
  const newMintStart = args
    .nextU64()
    .expect('mintStart argument is missing or invalid');

  Storage.set(MINT_START_TS, u64ToBytes(newMintStart));
}
...
export function mint(binaryArgs: StaticArray<u8>): void {
  // check mint start
  const mintStartTS = bytesToU64(Storage.get(MINT_START_TS));
  assert(Context.timestamp() >= mintStartTS, 'Too early')
